"""Resolve the ux_creator package and tools image, then exec inside Docker.

Plugin installs update the assets under plugins/ux but do not reinstall
the `ux_creator` Python package, and the host interpreter is not guaranteed to
carry the package dependencies. The launcher therefore runs every ux_creator
module inside the pinned ux-tools image, mounting the resolved source
tree and the workspace so paths stay identical inside the container.

Source resolution order (first directory containing ux_creator/__init__.py wins):
  1. $UX_SRC
  2. newest ~/.openhands/cache/extensions/ux-creator-agent-*/src
  3. /opt/ux/src (ux_creator-server image)
...
Image resolution order (first hit wins):
  1. $UX_TOOLS_IMAGE (full ref, e.g. ghcr.io/.../ux-tools@sha256:...)
...
Usage: mcp_server | prewarm | <ux_creator cli args...>. Any argument other
than mcp_server/prewarm is forwarded to `python -m ux_creator.cli` inside the
...
_MODULES = {
    "mcp_server": ("ux_creator.mcp_server",),
}

_CONTAINER_SRC = "/plugin-src"
_ENV_PREFIXES = ("OPENHANDS_", "UX_")
_ENV_KEYS = ("TMPDIR",)
    ...
    for home in _homes():
        ...
        try:
            if cache.is_dir():
                candidates.extend(
                    sorted(
                        cache.glob("ux-creator-agent-*/src"),
            ...
            pass
    candidates.append(Path("/opt/ux/src"))
...
def resolve_source(plugin_root: Path) -> Path | None:
    for candidate in _candidates(plugin_root):
        try:
            if (candidate / "ux_creator" / "__init__.py").is_file():
    ...
    for home in _homes():
        ...
        try:
            if cache.is_dir():
                dirs.extend(
                    sorted(
                        (p for p in cache.glob("ux-creator-agent-*") if p.is_dir()),
                        key=lambda path: path.stat().st_mtime,
                        reverse=True,
                    )
                )
...
def _image_from_lock(plugin_root: Path) -> str | None:
    ...
    for repo_dir in _repo_dirs(plugin_root):
        ref = _lock_entry_ref(repo_dir / "docker" / "image-digests.json", "ux_tools")
        if ref:
            return ref
    return None
...
def _ensure_image(plugin_root: Path, *, pull: bool = True) -> str:
    ...
    docker = _docker()
    if docker is None:
        raise RuntimeError("docker not found on PATH (ux_creator runs docker-only)")

    ref = os.environ.get("UX_TOOLS_IMAGE") or _image_from_lock(plugin_root)
    if ref is None:
        raise RuntimeError(
            "no ux_creator tools image resolvable: set UX_TOOLS_IMAGE or pin "
    ...
    if not pull:
        raise RuntimeError(
            f"ux_creator tools image {ref} not pulled locally; "
            "run 'ux_launcher.py prewarm' to fetch it"
        )
    print(f"ux_launcher: pulling tools image {ref}", file=sys.stderr)
    if (
        subprocess.run(
            [docker, "pull", ref],
            check=False,
        ...
        return ref
    raise RuntimeError(f"ux_creator tools image {ref} not present locally and pull failed")
...
def _warn_or_die(message: str, module_args: list[str]) -> int:
    if "--warn" in module_args:
        ...
        return 0
    print(f"ux_launcher: {message}", file=sys.stderr)
    return 1
...
def main() -> int:
    ...
    if not argv:
        print(
            "usage: ux_launcher.py {mcp_server|prewarm|<ux_creator cli args...>}",
            file=sys.stderr,
        )
        return 2
    ...
    if argv[0] == "prewarm":
        print(f"ux_launcher: tools image ready: {image}")
        return 0
    ...
    if argv[0] in _MODULES:
        ...
        inner = ["python", "-m", "ux_creator.cli", *argv]
