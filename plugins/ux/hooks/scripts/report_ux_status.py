#!/usr/bin/env python3
"""Report deterministic ux-report verdicts when an agent stops."""
...
def _find_reports(root: Path) -> list[Path]:
    reports: list[Path] = []
    for path in root.rglob("ux-report.json"):
        try:
...
def main() -> int:
    try:
        ...
        if statuses:
            ...
            context = f"No UX reports found under {working_dir}."
        print(json.dumps({"decision": "allow", "additionalContext": context}))
        return 0
    except Exception as exc:
        print(f"report_ux_status: {exc}", file=sys.stderr)
        return 1
